...
const splitColumnsEl = document.getElementById("splitColumns");
...
function renderTabsLayout(category, activeSubcategory) {
  ...
  category.subcategories.forEach((subcategory) => {
    ...
    tabNavEl.appendChild(button);
  });
  tabGridEl.innerHTML = activeSubcategory.tags.map((tag) => renderTagCard(category.id, tag)).join("");
}
    ...
    .map((subcategory) => {
      return `
        <section class="subcategory-column">
          <h4 class="subcategory-column-title">${subcategory.title}</h4>
